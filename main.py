class CaiT(keras.Model):
    def __init__(self,
        ...
        self.pos_embed = tf.Variable(tf.zeros((1, num_patches, dim)))
        ...
        dpr = [sd_prob for _ in range(n_self_attention)]
        ...
        self.ca_blocks = [
            LayerScale_Block_CA(dim=dim, 
                        ...
                        for i in range(n_class_attention2)
                    ]
        ...
        self.classification_head = Dense(self.num_classes)
    ...
    def call(self, x):
        ...
        for ca_block in self.ca_blocks:
            cls_emb, attn_score = ca_block(x, cls_token)
        ...
        if self.gobal_pool:# here, we are using all token's representation
